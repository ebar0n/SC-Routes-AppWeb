...
public class Application extends Controller {
	...
	@Restrict(@Group(Application.USER_ROLE))
	public Result createroutes() {
		final User localUser = this.userProvider.getUser(session());
		return ok(createroutes.render(this.userProvider, localUser));
	}

	@Restrict(@Group(Application.USER_ROLE))
	public Result view_support() {
		final User localUser = this.userProvider.getUser(session());
		return ok(view_support.render(this.userProvider, localUser));
	}
	...
	public Result support() {
		return ok(support.render(this.userProvider));
	}

	public Result findrutas() {
		return ok(findrutas.render(this.userProvider));
	}
	...
	public Result doSignup() {
		...
		if (filledForm.hasErrors()) {
			...
			// signup
			this.provider.handleSignup(ctx());
			return this.provider.handleLogin(ctx());
		}
	}
}